...
class Cart(object):
    ...
    def add(self, product, size, quantity=1, update=False):
        ...
        new_product = {'product_id': str(
            product.id), 'quantity': 1, 'price': str(product.price), 'size': size}
            ...
            for item in self.cart:
                if item['product_id'] == str(product.id) and item['size'] == size:
                    if update:
                        ...
                        if (stock.quantity - item['quantity'] - quantity) < 0:
                            return False
                        else:
                            item['quantity'] += 1
                    self.save()
    ...
    def __iter__(self):
        ...
        for item in self.cart:
            ...
            # filter stock where quantity is more than 0 for the selected product
            stock = Stock.objects.filter(product=product, quantity__gt=0)
            # get available sizes for the product, remove duplicates and sort by size field
            item['sizes_available'] = sorted(
                set([s.size for s in stock]), key=lambda k: k.size)
